  ...
  return (
    <div className="playground-main-content" style={{ flex: 1, display: 'flex', flexDirection: 'row', height: '100%', minHeight: 0, overflow: 'hidden' }}>
      ...
      <div className="playground-editor-panel" style={{ width: '60%', height: '100%', minWidth: 300, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div className="playground-editor-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: '#f9fafb', borderBottom: '1px solid #e5e7eb', fontSize: '0.8rem', color: '#6b7280', minHeight: 56 }}>
          <div style={{ padding: '4px 12px', backgroundColor: '#fff', borderRadius: '4px 4px 0 0', borderBottom: '2px solid #10a37f', color: '#111', fontWeight: 500, fontSize: '0.75rem' }}>playground.js</div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button type="button" onClick={handleRun} disabled={!code.trim() || running} title="Run (Ctrl+Enter)" style={{ background: code.trim() && !running ? '#10a37f' : '#d1d5db', color: code.trim() && !running ? '#fff' : '#9ca3af', border: 'none', borderRadius: 6, padding: '6px 12px', fontSize: '0.75rem', fontWeight: 500, cursor: code.trim() && !running ? 'pointer' : 'not-allowed', minWidth: 60, height: 28 }}>Run</button>
            <button type="button" onClick={handleCopy} disabled={!code.trim()} title="Copy code" style={{ background: code.trim() ? '#3b82f6' : '#d1d5db', color: code.trim() ? '#fff' : '#9ca3af', border: 'none', borderRadius: 6, padding: '6px 12px', fontSize: '0.75rem', fontWeight: 500, cursor: code.trim() ? 'pointer' : 'not-allowed', minWidth: 60, height: 28 }}>Copy</button>
          </div>
        </div>
      ...
      <div className="playground-output-panel" style={{ width: '40%', height: '100%', minWidth: 200, display: 'flex', flexDirection: 'column', background: '#fff', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: '#f9fafb', borderBottom: '1px solid #e5e7eb', fontSize: '0.8rem', color: '#6b7280', minHeight: 56 }}>
          <div style={{ padding: '4px 12px', backgroundColor: '#fff', borderRadius: '4px 4px 0 0', fontSize: '0.75rem', fontWeight: 500, color: '#111', border: '1px solid #e5e7eb', borderBottom: '2px solid #10a37f', marginBottom: -1 }}>Terminal Output</div>
        </div>
